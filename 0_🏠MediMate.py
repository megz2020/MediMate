...
new_google_cloud_credentials = None
...
if use_env:
    ...
    new_google_cloud_credentials = os.getenv('GOOGLE_APPLICATION_CREDENTIALS')
    openai_key = os.getenv('OPENAI_KEY')
    ...
    google_cloud_credentials =st.sidebar.file_uploader('Upload Google Cloud Credentials')
    st.sidebar.markdown('---')
    ...
    if submit:
        st.write('Submitted')
        new_google_cloud_credentials = os.path.join(os.getcwd(), google_cloud_credentials.name)
        if new_google_cloud_credentials:
            save_uploaded_file(google_cloud_credentials, new_google_cloud_credentials)
        else:
            new_google_cloud_credentials = os.environ.get('GOOGLE_APPLICATION_CREDENTIALS')
    ...
    embeddings = OpenAIEmbeddings(openai_api_key=openai_key)
    credentials, llm = init_google_service_account_client(new_google_cloud_credentials)
